...
data = pd.read_excel(
                os.path.join(
                        ...
                        ),'Описание отраслей.xlsx'
                    ))

data
data = data.loc[:, ["ТНВЭД", "Товар", "Группа товаров"]]
data = data.loc[data["ТНВЭД"].str.match("\d+"),:].fillna("")



all_commodity_ids = [i[0] for i in session_test.query(Commodity_code_RU.id).all()]
data = data.loc[~data["ТНВЭД"].isin(all_commodity_ids),:]


# for index, row in data.iterrows():
#    print(row['ТНВЭД'], row['Товар'], row['Группа товаров'])


for index, row in data.iterrows():
   commodity_ru_entry = {"id": str(row['ТНВЭД']),
                           "commodity_name": row['Товар'],
                           "product_group": row['Группа товаров']}

   adding_commodity_ru = Commodity_code_RU(**commodity_ru_entry)
   session_test.add(adding_commodity_ru)
session_test.commit()
   # print(commodity_ru_entry)

# dir(session_test.query(exists().where(Commodity_code_RU.id.in_(data["ТНВЭД"]))))



# session_test.query(Commodity_code_RU).filter(tuple_(Commodity_code_RU.id).in_(list(data["ТНВЭД"])))
